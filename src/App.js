...
function exportCSV(entries, employees, filename) {
  const rows = [["Nome", "Data", "Entrata", "Uscita", "Ore lavorate", "Tipo", "Luogo", "Nota"]];
  ...
  return (
    <>
      ...
      <div style={{ background: C.surface, borderRadius: 12, padding: "13px 15px", border: "1.5px solid " + C.border, borderLeft: "4px solid " + (e.gps ? C.accent : C.borderStrong) }}>
        ...
        {e.luogo && <div style={{ marginTop: 4, fontSize: 11, color: C.accent, fontFamily: "monospace", background: C.accentLight, padding: "4px 8px", borderRadius: 6, display: "inline-block" }}>📍 {e.luogo}</div>}
      </div>
    </>
  );
}
...
function EmployeeApp({ emp, entries, onLogout, onResetPwd }) {
  ...
  const [mLuogo, setMLuogo] = useState("");
  ...
  const stopTimer = async () => {
    ...
    setTotalPaused(0);
    setElapsed(0);
    await saveToFirebase({ employeeId: emp.id, employeeName: empName, dateIn, dateOut, type: "timer", gps: geo.coords, nota: nota || null, luogo: luogo || null, pauseMs: totPaused });
  ...
  const doSaveManual = async () => {
    const dateIn = new Date(mDate + "T" + mIn).toISOString();
    const dateOut = new Date(mDate + "T" + mOut).toISOString();
    await saveToFirebase({ employeeId: emp.id, employeeName: empName, dateIn, dateOut, type: "manual", nota: mNota || null, luogo: mLuogo || null });
    setMDate(""); setMIn(""); setMOut(""); setMNota(""); setMLuogo("");
      ...
      <div style={S.card}>
              ...
              {activeTimer ? (
                <>
                  ...
                  <div style={{ marginBottom: 16, textAlign: "left" }}>
                    <label style={S.label}>Luogo</label>
                    <select value={luogo} onChange={e => setLuogo(e.target.value)} style={S.select}>
        ...
        {tab === "manual" && (
          <div style={{ display: "flex", flexDirection: "column", gap: 14 }}>
            ...
            <div>
              <label style={S.label}>Luogo</label>
              <select value={mLuogo} onChange={e => setMLuogo(e.target.value)} style={S.select}>
                <option value="">— Seleziona luogo —</option>
                <option value="Agriturismo">Agriturismo</option>
                <option value="Villa">Villa</option>
                <option value="Casa">Casa</option>
...
function Dashboard({ entries, employees }) {
  ...
  const filtered = entries.filter(e => {
    ...
    if (fLuogo !== "all" && e.luogo !== fLuogo) return false;
    return true;
  });
  ...
  return (
    <div style={{ display: "flex", flexDirection: "column", gap: 12 }}>
      ...
      <div style={{ ...S.card, margin: "0 16px" }}>
        ...
        <label style={S.label}>Luogo</label>
        <div style={{ display: "flex", gap: 6, marginBottom: 14, flexWrap: "wrap" }}>
          {[["all", "Tutti"], ["Agriturismo", "Agriturismo"], ["Villa", "Villa"], ["Casa", "Casa"]].map(([val, label]) => (
            <button key={val} onClick={() => setFLuogo(val)} style={{ padding: "6px 12px", borderRadius: 20, fontSize: 11, fontFamily: "monospace", background: fLuogo === val ? C.accent : C.surfaceAlt, color: fLuogo === val ? "#fff" : C.textSub, border: "1.5px solid " + (fLuogo === val ? C.accent : C.border), cursor: "pointer", fontWeight: 600 }}>{label}</button>
          ))}
        </div>
...
function ManagerView({ entries, employees }) {
  ...
  const filtered = entries.filter(e => {
    ...
    if (fFrom && new Date(e.dateIn) < new Date(fFrom)) return false;
    if (fTo && new Date(e.dateIn) > new Date(fTo + "T23:59:59")) return false;
    if (fLuogoM !== "all" && e.luogo !== fLuogoM) return false;
    return true;
          ...
          <div style={{ ...S.card, margin: "0 16px" }}>
            ...
            <div style={{ marginTop: 12 }}>
              ...
              <div style={{ display: "flex", gap: 6, flexWrap: "wrap" }}>
                {[["all", "Tutti"], ["Agriturismo", "Agriturismo"], ["Villa", "Villa"], ["Casa", "Casa"]].map(([val, label]) => (
                  <button key={val} onClick={() => setFLuogoM(val)} style={{ padding: "6px 12px", borderRadius: 20, fontSize: 11, fontFamily: "monospace", background: fLuogoM === val ? C.accent : C.surfaceAlt, color: fLuogoM === val ? "#fff" : C.textSub, border: "1.5px solid " + (fLuogoM === val ? C.accent : C.border), cursor: "pointer", fontWeight: 600 }}>{label}</button>
                ))}
              </div>
            </div>
          </div>
