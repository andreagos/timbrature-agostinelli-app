...
function Dashboard({ entries }) {
  ...
  const [fEmps, setFEmps] = useState(EMPLOYEES.map(e => e.id));
  ...
  const filtered = entries.filter(e => {
    ...
    if (!fEmps.includes(e.employeeId)) return false;
  ...
  const empStats = EMPLOYEES.map(emp => {
    ...
    const timerMsEmp = timerEs.reduce((s, e) => s + (new Date(e.dateOut) - new Date(e.dateIn)), 0);
    const manualMsEmp = manualEs.reduce((s, e) => s + (new Date(e.dateOut) - new Date(e.dateIn)), 0);
    return { ...emp, ms, count: es.length, timerCount: timerEs.length, manualCount: manualEs.length, timerMs: timerMsEmp, manualMs: manualMsEmp };
  }).sort((a, b) => b.ms - a.ms);
    ...
    <div style={{ display: "flex", flexDirection: "column", gap: 12 }}>
      ...
      <div style={{ ...S.card, margin: "0 16px" }}>
        ...
        <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 10, marginBottom: 14 }}>
          <div>
            ...
            <input type="date" value={fFrom} onChange={e => setFFrom(e.target.value)} style={S.input} />
          </div>
          <div>
            <label style={S.label}>Al</label>
            <input type="date" value={fTo} onChange={e => setFTo(e.target.value)} style={S.input} />
          </div>
        </div>
        ...
        <div style={{ display: "flex", gap: 6, marginBottom: 14 }}>
          {[["all", "Tutte"], ["timer", "📍 Timer GPS"], ["manual", "✏️ Manuali"]].map(([val, label]) => (
        ...
        <div style={{ display: "flex", flexDirection: "column", gap: 6 }}>
          {EMPLOYEES.map(emp => (
            <button key={emp.id} onClick={() => toggleEmp(emp.id)} style={{
              ...
              <span style={{ fontSize: 13, fontWeight: 600, color: C.text }}>{emp.name}</span>
            </button>
          ))}
        </div>
      </div>
      ...
      <div style={{ ...S.card, margin: "0 16px" }}>
        ...
        <div style={{ display: "flex", flexDirection: "column", gap: 2 }}>
          {empStats.filter(s => fEmps.includes(s.id)).map((s, i) => (
            <div key={s.id} style={{
              padding: "14px 0",
              borderBottom: i < empStats.filter(x => fEmps.includes(x.id)).length - 1 ? "1px solid " + C.border : "none",
            }}>
              ...
              <div style={{ display: "flex", gap: 12, flexWrap: "wrap" }}>
                ...
                {fType !== "manual" && s.timerCount > 0 && (
                  <span style={{ fontSize: 11, color: C.accent, fontFamily: "monospace" }}>
                    📍 {s.timerCount} GPS · {formatDuration(s.timerMs)}
                  </span>
                )}
                {fType !== "timer" && s.manualCount > 0 && (
                  <span style={{ fontSize: 11, color: "#4338CA", fontFamily: "monospace" }}>
                    ✏️ {s.manualCount} manuali · {formatDuration(s.manualMs)}
                  </span>
                )}
              </div>
            </div>
          ...
          {filtered.length === 0 && (
            <div style={{ color: C.textMuted, textAlign: "center", padding: "20px 0", fontSize: 13, fontFamily: "monospace" }}>
